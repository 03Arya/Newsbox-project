export default (function () {
    ...
    async function fetchNYTHealthArticles() {
        try {
            const response = await fetch('https://api.nytimes.com/svc/topstories/v2/travel.json?api-key=' + apiKey);
            ...
            document.querySelector('.newsArticleTextTravel').innerHTML = articleContent;
        } catch (error) {
            console.error('Error fetching articles:', error);
            document.querySelector('.newsArticleTextTravel').innerText = 'Failed to fetch articles.';
        }
    ...
    const DROPDOWN = document.querySelector("#travelDropdown");
    DROPDOWN.addEventListener("click", clickHandler);
  
  
    function clickHandler() {
      var ARTICLE = document.querySelector(".newsArticleTravel");
