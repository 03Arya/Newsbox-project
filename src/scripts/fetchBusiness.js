export default (function () {
    ...
    async function fetchNYTHealthArticles() {
        try {
            ...
            document.querySelector('.newsArticleTextBusiness').innerHTML = articleContent;
        } catch (error) {
            console.error('Error fetching articles:', error);
            document.querySelector('.newsArticleTextBusiness').innerText = 'Failed to fetch articles.';
        }
    ...
    const DROPDOWN = document.querySelector("#businessDropdown");
    DROPDOWN.addEventListener("click", clickHandler);
  
  
    function clickHandler() {
      var ARTICLE = document.querySelector(".newsArticleBusiness");
